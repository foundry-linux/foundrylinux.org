...
|---|---|---|
| `apt.foundrylinux.org` | Foundry Linux distro toolchain — `foundry-*` edition/category metapackages + **26 vendored upstreams** (`f9dasm`, `ghidra`, `halfempty`, `libvgm`, `vgmstream`, `ppsspp`, `snes9x-gtk`, `task`, `ruff`, `python3-{glfw,inators,librosa,mss,picire,pydub}`, `blender-asset-finder{,-cli}`, `asar-snes-assembler`, `bsnes-jg`, `drmon`, `ldtk`, `m8te`, `mesen2`, `tilemap-studio`, `wla-dx`, `pvsneslib` — full list in [`foundry-apt/LICENSES-VENDORED.md`](foundry-apt/LICENSES-VENDORED.md)) | this repo, `foundry-apt/` |
