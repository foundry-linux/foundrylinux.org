...
|---|---|---|
| `apt.foundrylinux.org` | Foundry Linux distro toolchain — `foundry-*` edition/category metapackages + **20 vendored upstreams** (`f9dasm`, `ghidra`, `libvgm`, `vgmstream`, `ppsspp`, `snes9x-gtk`, `task`, `ruff`, `python3-{glfw,librosa,mss,pydub}`, `blender-asset-finder{,-cli}`, `asar`, `ldtk`, `m8te`, `tilemap-studio`, `wla-dx`, `pvsneslib` — full list in [`foundry-apt/LICENSES-VENDORED.md`](foundry-apt/LICENSES-VENDORED.md)) | this repo, `foundry-apt/` |
