 ...
 *
 * DESCRIPTION
 *		Displays a dialog for Room or Tileset Info
 *
 * HISTORY
 *
*/
...
LOCAL short		 TileWidth = 16;
LOCAL short		 TileHeight = 16;
LOCAL short		 RoomWidth = 20;
LOCAL short		 RoomHeight = 12;

LOCAL ListType	 TileTypeListX;
ListType	*TileTypeList = &TileTypeListX;
LOCAL ListType	 RoomTypeListX;
ListType	*RoomTypeList = &RoomTypeListX;

char			*RoomTypeGroup = "No Room Type Group";
char			*TileTypeGroup = "No Tile Type Group";
...
#define NUM_TYPE_NDX	0
#define NUM_IDNUM_NDX	1
...
LOCAL char	NumStrings[4][EIO_MAXPATH];
 ...
 *
 * PURPOSE
 *		Search <TileTypeList> for record with same Type as <FindType>,
 *		return corresponding <Info[<iInfoN>]> record.
 *
 *		Return -1 if not found.
 *
 * INPUT
 *		FindType	: tileset type to match
 *		iInfoN		: index of which info to return 
 *
 * EFFECTS
 *
 *
 * RETURN VALUE
 *		Return Info[<iInfoN] for tileset type <FindType>.
 *
 * HISTORY
 *		12/14/93 Tuesday (dcc) - added <iInfoN> parameter.
 *
 * SEE ALSO
 *
*/
int LookupInfoN(int FindType, int iInfoN)
{
	RT_Type *prtt;

/*-------------------------------------------------------------------------*/
#if FUNC_NAMES
 CurrentFuncName = "LookupInfoN";
#endif
#if REQUIRE
...
/*-------------------------------------------------------------------------*/
	...
	prtt = (RT_Type *) Head(TileTypeList);

	while (!IsEOList(prtt))
	{
		if (prtt->Type == FindType)
			return prtt->Info[iInfoN];

		prtt = (RT_Type *) Next(prtt);
	}
	return -1;
...
/*********************************************************************
 *
 * LookupSomeTSTInfoFromUserType
 *
 * PURPOSE
 *	Search <TileTypeList> for record with same Type as <ptst->UserType>,
 *	and set the following fields in <ptst>:
 *
 *	ptst->LayerToUse
 *	ptst->ubShowFlagAND
 *	ptst->cxDisplay
 ...
 *	ptst->ctilyDispE
 *
 *	Default values are supplied if <ptst->UserType> is not found in
 *	<TileTypeList>.
 *
 *	NOTE that these values are currently NOT saved in the tUME IFF file!
 *
 * INPUT
 *	ptst->UserType	: get values for this user type
 *	ptst->Width		: copied to ptst->cxDisplay if looked-up value == -1
 *	ptst->Height	: copied to ptst->cyDisplay if looked-up value == -1
 *
 * ASSUMES
 *	
 *
 * RETURN VALUE
 *	NONE.
 *	Fields in <ptst> listed above filled out.
 *
 * HISTORY
 *	12/14/93 Tuesday (dcc) - created.
 *
*/
void LookupSomeTSTInfoFromUserType(TileSetType *ptst)
{
	RT_Type *prtt;

/*-------------------------------------------------------------------------*/
#if FUNC_NAMES
 CurrentFuncName = "LookupSomeTSTInfoFromUserType";
#endif
#if REQUIRE
...
/*-------------------------------------------------------------------------*/
	...
	while (!IsEOList(prtt))
	{
		if (prtt->Type == ptst->UserType)
		{
			ptst->LayerToUse	= (short) prtt->Info[itstLayer];
			ptst->ubShowFlagAND	= (UBYTE) prtt->Info[itstFlag];
			ptst->fAlwaysTransp	=  (BOOL) prtt->Info[itstTransparent];
			ptst->cxDisplay		= prtt->Info[itstcxDisplay];
			if (ptst->cxDisplay == -1)
				ptst->cxDisplay = ptst->Width;
			ptst->cyDisplay		= prtt->Info[itstcyDisplay];
			if (ptst->cyDisplay == -1)
				ptst->cyDisplay = ptst->Height;
			ptst->ctilxSkip		= prtt->Info[itstctilxSkip];
			ptst->ctilxDispO	= prtt->Info[itstctilxDispO];
			ptst->ctilyDispO	= prtt->Info[itstctilyDispO];
			ptst->ctilxDispE	= prtt->Info[itstctilxDispE];
			ptst->ctilyDispE	= prtt->Info[itstctilyDispE];
			return;
		}

		prtt = (RT_Type *) Next(prtt);
	}
	/* Fill structure with default values (see TYPELIST#InfoDefault[]): */

	ptst->LayerToUse	= LayerToUseDefault;
	ptst->ubShowFlagAND = ubShowFlagDefault;
	ptst->fAlwaysTransp	= fTransparentDefault;
	ptst->cxDisplay		= ptst->Width;
	ptst->cyDisplay		= ptst->Height;
	ptst->ctilxSkip		= ctilxSkipDefault;
...
/*-------------------------------------------------------------------------*/

	{
		...
		count = 0;
		type  = atoi (NumStrings[NUM_TYPE_NDX]);
		rt    = (RT_Type *) Head (RTList);
		while (!EndOfList (rt)) {
			rt->Node.Priority = 0;
			if (rt->Type == type) {
				type = -30000;
...
#define LIST_GAD	1
	...
	{ RTDLG_NUM_LEFT + ARROW_WIDTH, RT_TYPE_TOP, NUMBER_WIDTH - 2, NUMBER_HEIGHT,
		HTA_DOWN | HTA_REPEAT | HTA_INSURE_UP, 0, HighlightString, &NumString[NUM_TYPE_NDX], },
	{ RTDLG_NUM_LEFT + ARROW_WIDTH + NUMBER_WIDTH - 1, RT_TYPE_TOP, ARROW_WIDTH, ARROW_HEIGHT,
		HTA_COMPLEMENT, 0x00, IncDecNumber, },

	/* IDNum Number */
	{ RTDLG_NUM_LEFT, RT_NUM_TOP, ARROW_WIDTH, ARROW_HEIGHT,
		HTA_COMPLEMENT, 0x11, IncDecNumber, },
	{ RTDLG_NUM_LEFT + ARROW_WIDTH, RT_NUM_TOP, NUMBER_WIDTH - 2, NUMBER_HEIGHT,
		HTA_DOWN | HTA_REPEAT | HTA_INSURE_UP, 0, HighlightString, &NumString[NUM_IDNUM_NDX], },
	{ RTDLG_NUM_LEFT + ARROW_WIDTH + NUMBER_WIDTH - 1, RT_NUM_TOP, ARROW_WIDTH, ARROW_HEIGHT,
 ...
 *
 * SYNOPSIS
 *		short RoomNTileInfoReq (RoomType *room, TileSetType *tst,
 *						short *pctilx, short *pctily, short fSetCtil)
 *
 * PURPOSE
 *		
 *
 ...
 *		fSetCtil:	set TRUE to let user change width & height
 *
 * EFFECTS
 *
 *
 ...
 *
 * HISTORY
 *		04/11/92 Saturday (dcc) - return FALSE if user canceled.
 *		04/11/92 Saturday (dcc) - if (new), yet tst->width != 0,
 *									use old tst->width in requester.
 *		04/14/92 Tuesday (dcc) - KLUDGED so if no item is selected
 *									in list box, it doesn't draw in upper-
 *									left corner of screen.
 *		08/07/92 Friday (dcc) - set <RoomWidth> & <RoomHeight> if
 *									values coming in are non-zero.
 *									This lets the user re-size a room.
 *
 * SEE ALSO
 *
*/
...
/*-------------------------------------------------------------------------*/
#if FUNC_NAMES
 CurrentFuncName = "RoomNTileInfoReq";
#endif
#if REQUIRE
...
/*-------------------------------------------------------------------------*/

	{
		...
		if (tst) {
			itoa (tst->UserType, NumStrings[NUM_TYPE_NDX], 10);
			itoa (tst->UserNumber, NumStrings[NUM_IDNUM_NDX], 10);
			top    = TS_TOP;
			height = TS_HEIGHT;
			RTList = TileTypeList;
			RtStr[COMMENT_STR] = szTileComments;
		} else {
			itoa (room->UserType, NumStrings[NUM_TYPE_NDX], 10);
			itoa (room->UserNumber, NumStrings[NUM_IDNUM_NDX], 10);
			top    = RTDLG_TOP;
			height = RTDLG_HEIGHT;
			RTList = RoomTypeList;
			RtStr[COMMENT_STR] = szRoomComments;
		}
		...
		if (PushWindowClipValues (RTWin)) {
			...
			if (InitTextLGAD (
				...
				DrawString (RTFont, RTDLG_LEFT + LABEL_LEFT, top + RT_TYPE_TOP + 2, "User Type");
				DrawString (RTFont, RTDLG_LEFT + LABEL_LEFT, top + RT_NUM_TOP + 2, "User Number");

				/*
				** If new draw Width and Height stuff.
				*/
				if (fSetCtil) {

					StartEPicture (&rp, &NumEPicture, RTDLG_LEFT + RTDLG_NUM_LEFT, top + RT_WIDTH_TOP);
					StartEPicture (&rp, &NumEPicture, RTDLG_LEFT + RTDLG_NUM_LEFT, top + RT_HEIGHT_TOP);

				} else {
				}

				/*
				** If new or not add or remove Width/Height areas.
				*/
				{
					short	i;

					for (i = 0; i < 6; i++) {
						RTHitArea[WIDTH_GADS + i].Width = (fSetCtil) ? NewGadWidths[i] : 0;
					}
					for (i = 6; i < 7; i++) {
						RTHitArea[WIDTH_GADS + i].Width = (tst) ? NewGadWidths[i] : 0;
					}
				}

				if (tst) {
					/*
					** If Tilesets draw NTI.
					*/
					strcpy (Naem, tst->Filespec);
					SaveGrafx = !(tst->Flags & DONT_SAVEIMAGES);

					/* Draw comment stuff */

					if (tst->Comment[0]) {
						strcpy (Com1, tst->Comment[0]);
					} else {
						Com1[0] = '\0';
					}

					if (tst->Comment[1]) {
						strcpy (Com2, tst->Comment[1]);
					} else {
						Com2[0] = '\0';
					}

					StartEPicture (&rp, &TSetEPicture, RTDLG_LEFT, top);
					DrawString (RTFont, RTDLG_LEFT + RTDLG_WIDTH / 2 - StringLen (RTFont, "Tileset Info") / 2, top + 2, "Tileset Info");
					if (fSetCtil) {
						if (tst->Width == 0 && tst->Height == 0)
						{
							itoa (TileWidth, NumStrings[NUM_WIDTH_NDX], 10);
							itoa (TileHeight, NumStrings[NUM_HEIGHT_NDX], 10);
						}
						else
						{
							itoa (tst->Width, NumStrings[NUM_WIDTH_NDX], 10);
							itoa (tst->Height, NumStrings[NUM_HEIGHT_NDX], 10);
						}

						DrawString (RTFont, RTDLG_LEFT + LABEL_LEFT, top + RT_WIDTH_TOP + 2, "Tile Width");
						DrawString (RTFont, RTDLG_LEFT + LABEL_LEFT, top + RT_HEIGHT_TOP + 2, "Tile Height");
					} else {
						/*
						** Print some tset info?
						*/
					}
					UpdateNTI ();
				} else {
					/*
					** If Rooms remove NTI area.
					*/
					if (room->Name) {
						strcpy (Naem, room->Name);
					} else {
						Naem[0] = '\0';
					}
					...
					if (room->Comment[0]) {
						strcpy (Com1, room->Comment[0]);
					} else {
						Com1[0] = '\0';
					}

					if (room->Comment[1]) {
						strcpy (Com2, room->Comment[1]);
					} else {
						Com2[0] = '\0';
					}

					DrawString (RTFont, RTDLG_LEFT + RTDLG_WIDTH / 2 - StringLen (RTFont, "Room Info") / 2, top + 2, "Room Info");
					if (fSetCtil) {
						itoa (RoomWidth, NumStrings[NUM_WIDTH_NDX], 10);
						itoa (RoomHeight, NumStrings[NUM_HEIGHT_NDX], 10);

						DrawString (RTFont, RTDLG_LEFT + LABEL_LEFT, top + RT_WIDTH_TOP + 2, "Room Width");
						DrawString (RTFont, RTDLG_LEFT + LABEL_LEFT, top + RT_HEIGHT_TOP + 2, "Room Height");
					} else {
						/*
						** Print some room info
						*/
					}
				}

				/*
				** Set String Gadget heights and tab order.
				*/
				{
					short i;

					for (i = 0; i < NUM_STRING_GADS ; i++) {
						NumString[i].Y    = top + StrHeights[i];
						NumString[i].Next = ESTable[((fSetCtil) ? 0 : 1)][i];
					}
				}

				UpdateStrings (MainWindow, &NumString[0], ((fSetCtil) ? 7 : 5), NULL);
...
dRTcleanup:
		...
		if (RTsuccess) {
			...
			if (tst) {
				tst->Flags &= (~DONT_SAVEIMAGES);
				tst->Flags |= (SaveGrafx) ? 0 : DONT_SAVEIMAGES;
				repstr (&tst->Filespec, Naem);
				repstr (&tst->Comment[0], Com1);
				repstr (&tst->Comment[1], Com2);
				tst->UserType   = atoi (NumStrings[NUM_TYPE_NDX]);
				tst->UserNumber = atoi (NumStrings[NUM_IDNUM_NDX]);
				if (fSetCtil) {
					tst->Width  = atoi (NumStrings[NUM_WIDTH_NDX]);
					tst->Height = atoi (NumStrings[NUM_HEIGHT_NDX]);
				}
			} else {
				repstr (&room->Name, Naem);
				repstr (&room->Comment[0], Com1);
				repstr (&room->Comment[1], Com2);
				room->UserType   = atoi (NumStrings[NUM_TYPE_NDX]);
				room->UserNumber = atoi (NumStrings[NUM_IDNUM_NDX]);
				if (fSetCtil) {
					*pctilx = atoi (NumStrings[NUM_WIDTH_NDX]);
					*pctily = atoi (NumStrings[NUM_HEIGHT_NDX]);
				}
			}
		}
