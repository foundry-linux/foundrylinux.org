...
# (Cloudsmith) and install-foundry-retro-tools.sh (apt.foundrylinux.org).
#
# --skip-blender fallback path: install-foundry-engine-build-deps.sh
...
Default path:  apt install worldfoundry-development + task + foundry-retro-tools
...
Options:
  ...
  --skip-retro      Skip foundry-retro-tools
...
if $SKIP_BLENDER; then
    ...
    run_subscript install-foundry-engine-build-deps.sh "${DRY_FLAG[@]}"
...
if $SKIP_RETRO; then
    ...
    run_subscript install-foundry-retro-tools.sh "${retro_args[@]}"
fi
