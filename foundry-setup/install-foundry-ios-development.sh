...
# foundry-ios-development metapackage on apt.foundrylinux.org
...
for arg in "$@"; do
    case "$arg" in
        -h|--help)
            cat <<EOF
Phase 0 installer for foundry-ios-development

Installs Linux-side iOS development tooling:
  apt (apt.foundrylinux.org → foundry-ios-development):
...
step "Installing foundry-ios-development"
apt_update
run_sudo apt-get install -y foundry-ios-development pipx
ok "foundry-ios-development installed"
