...
| KDE color scheme | `includes.chroot/usr/share/color-schemes/` + `kdeglobals` | `FoundryLinux.colors` |
...
Place it in `config/includes.chroot/usr/share/color-schemes/FoundryLinux.colors`
and point `kdeglobals` at it via `config/includes.chroot/etc/xdg/kdeglobals`.

**`FoundryLinux.colors`** (KColorScheme INI format):
```ini
[ColorEffects:Disabled]
# ... (generated by KDE color scheme editor or kcolorscheme tool)

[Colors:Button]
BackgroundNormal=30,30,30
ForegroundNormal=240,240,240
# ... brand palette entries
...
**`kdeglobals`** (system-wide color scheme pointer):
```ini
[General]
ColorScheme=FoundryLinux

[KDE]
LookAndFeelPackage=org.foundrylinux.foundry-linux
```

The `LookAndFeelPackage` entry also points Plasma at the Look-and-Feel
package, making `kdeglobals` the single system-wide wiring point for both.
