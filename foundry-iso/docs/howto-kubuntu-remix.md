...
| Wallpaper reverts to Kubuntu cones on live session | Polling for plasmashell DBus registration is not sufficient: plasmashell registers on DBus **before** it applies the Kubuntu Look-and-Feel defaults. Firing `plasma-apply-wallpaperimage` at that moment gets overwritten by the LAF step. | Poll for `~/.config/plasma-org.kde.plasma.desktop-appletsrc` to contain a `[Containments]` section — plasmashell writes that file only **after** the LAF wallpaper has been applied. Once that condition is met, `plasma-apply-wallpaperimage` fires last and wins. |
...
| Slideshow (carousel) text has no left margin despite `anchors.leftMargin` | Qt Quick forbids mixing positional properties (`x`, `width`) with anchor properties on the **same item** — the positional values are silently ignored. This also applies to `anchors.leftMargin` on a `Column` whose children set `width: parent.width`, creating a circular implicit-width dependency. | Wrap the `Column` in an `Item` with `anchors.fill: parent; anchors.leftMargin: N; anchors.rightMargin: N`. The `Item` gets an unambiguously margin-adjusted explicit width from the anchor engine; the `Column` and its `Text` children then use `width: parent.width` cleanly. |
