...
| `dpkg` conffile prompt halts the non-interactive build (`*** foo (Y/I/N/O/D/Z)? … end of file on stdin at conffile prompt`) | A package ships a file under `/etc/` (a conffile) **and** `dpkg-divert`s the same path. On upgrade the divert renames the path away, so dpkg sees the conffile as "deleted (by you or by a script) since installation" and prompts — the build has no TTY → EOF → abort. Also fires whenever a local-debs upgrade ships a conffile that differs from the published version. | **Divert-and-replace** (the `dash`/`bin/sh` pattern): ship your version under `/usr/share/<pkg>/` (**not** a conffile), `dpkg-divert --add --rename` the owner's copy in `preinst`, `cp` yours into place in `postinst`, then remove + undivert in `postrm`. Belt-and-suspenders: pass `-o Dpkg::Options::=--force-confdef -o Dpkg::Options::=--force-confnew` on the local-debs `apt-get install` so a published→local conffile diff never stalls the build. |
...
| Bootloader fails: `grub-install … returned error code 1` | `mount.conf` schema violations cause Calamares to silently drop the entire `extraMounts` block. Two bugs bit us: (1) `options:` must be a **YAML array** — `options: bind` is a scalar; Python's `",".join("bind")` = `"b,i,n,d"` and schema validation rejects it. (2) `extraMountsEfi:` **does not exist** — the Calamares 3.3 mount module schema has `additionalProperties: false`; the unknown key rejects the whole config. When the block is dropped, grub-install runs with no `/dev` in the target chroot and exits code 1. | Use `options: ["bind"]` (array). Put efivarfs in `extraMounts` with `efi: true` on the entry — never a separate `extraMountsEfi:` key. The `efi: true` flag makes Calamares skip the mount on non-EFI systems. |
| Bootloader fails: `grub-install … returned error code 1` (sub-cause: missing modules) | `grub-efi-amd64` (and its `-bin` companion) must be **inside the squashfs** so Calamares can call `grub-install` in the target chroot offline. If it's only listed in the static `foundry.list.chroot` but not in `config/auto/config`, `lb config` overwrites the list on every build and the package is silently dropped. | Add `echo "grub-efi-amd64" >> config/package-lists/foundry.list.chroot` to **`config/auto/config`** (the generated file), not only to the static file. |
| QEMU virtio-vga-gl live session starts at 640×480 | `virtio-vga-gl` reports 640×480 as the initial framebuffer size to the Wayland compositor regardless of the `xres=`/`yres=` QEMU args (those set the maximum, not the initial mode). KWin starts without a saved output config and picks the first/default mode. | Write `kwinoutputconfig.json` into `/etc/skel/.config/` via a chroot hook. KWin reads it on login and switches to the saved mode when the EDID hash matches. The QEMU virtio-vga EDID hash is stable (`2043e82…`). Also add a `foundry-display-resolution.desktop` autostart as belt-and-suspenders: `kscreen-doctor output.Virtual-1.mode.1280x1024` if current mode is 640×480. Real hardware won't match the EDID hash and is unaffected. (Hook `1100`, ISO 0.9.76+.) |
...
| Unwanted **swap chooser dropdown** appears on the partition page despite `userSwapChoices: []` | Calamares 3.3.x treats an **empty** `userSwapChoices: []` as *unconfigured* and backfills its built-in choices (none + suspend), so a "No swap / Swap with Hibernate" combo box appears. The combo is shown only when there is **> 1** choice. | Give `userSwapChoices` a **single entry** (`- none`) — one choice → nothing to pick → Calamares hides the combo and forces it. Keep `initialSwapChoice` as the forced value. **Default no-swap is intentional on a Secure-Boot distro**: Ubuntu kernel *lockdown* (triggered by Secure Boot, which `shim-signed` enables) **disables hibernation**, so a `suspend` swap is RAM-sized dead weight that can't actually hibernate; if OOM-during-build is a concern later, prefer a swapfile (`initialSwapChoice: "file"`) over a partition. |
...
5. **Tees the entire build to `dist/build-{edition}-{version}.log`** (`… 2>&1 | tee "$LOG"`) so the output is reviewable after the run.

**Always capture and review the build log — the failures that matter hide in it.** A multi-minute containerised build scrolls thousands of lines, and the things that actually break a build only ever appear in this captured output, never on a glanceable terminal: a hook exiting non-zero, a purged-package wallpaper regression, or a **`dpkg` conffile prompt that silently halts the whole non-interactive install** — e.g. `*** kicker-extra-favoritesrc (Y/I/N/O/D/Z) [default=N] ? … end of file on stdin at conffile prompt`. `task iso-build` captures the log automatically (via `tee`); when you invoke the build script **directly** — e.g. to bypass `apt-build` when an unrelated package is broken — redirect it yourself so you don't lose the output to the scrollback:

```bash
EDITION=anvil bash scripts/build-iso.sh > dist/build-anvil-$(cat VERSION).log 2>&1
grep -niE 'conffile|error|failed|halt|exit non-zero' dist/build-anvil-*.log   # scan for the needles
```
