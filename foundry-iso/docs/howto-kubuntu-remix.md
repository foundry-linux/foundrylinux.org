...
| Wallpaper reverts to Kubuntu default on live session | `plasma-apply-wallpaperimage` fires before plasmashell is DBus-registered. With `X-KDE-autostart-after=panel` this still happens: Plasma 6 doesn't honour that hint reliably. | Don't use `X-KDE-autostart-after=panel` alone. Wrap the call in a script that polls `dbus-send --session --dest=org.kde.plasmashell --print-reply /PlasmaShell org.kde.PlasmaShell.version` once per second (up to 60 s) and only calls `plasma-apply-wallpaperimage` after the DBus method succeeds. |
