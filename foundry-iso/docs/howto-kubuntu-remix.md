...
| XDG autostart app skipped on every boot (`Skipped due to 'exec-condition'`) | `X-KDE-autostart-condition` uses `kreadconfig6` to check a KConfig key. If the app guards re-show with its own sentinel file instead of writing that KConfig key, the condition sees the key absent (`""`) on first boot and every subsequent boot — condition never passes | **Remove `X-KDE-autostart-condition` if the app has its own sentinel logic.** The app starts unconditionally at login and exits silently (< 1 ms) when its sentinel exists. Pre-seeding the KConfig key only works if the app also updates that same key when done — mixing two sentinel mechanisms guarantees breakage |
