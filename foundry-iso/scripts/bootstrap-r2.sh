...
# ── step 8: fetch shared GPG key from foundry-secrets ───────────────────────
...
if $DRY_RUN; then
  ...
  info "[8] Fetching GPG_PRIVATE_KEY from r2://${SECRETS_BUCKET}/GPG_PRIVATE_KEY"
  GPG_PRIVATE_KEY=$(r2_get_secret "GPG_PRIVATE_KEY" || true)
  if [[ -z "$GPG_PRIVATE_KEY" ]]; then
    die "[8] Could not fetch GPG_PRIVATE_KEY from r2://$SECRETS_BUCKET — check token has R2 read access"
  fi
  export GPG_PRIVATE_KEY
  ok "[8] GPG_PRIVATE_KEY fetched"
else
