#!/usr/bin/env bash
# Vendored-upstream Debian package build wrapper for python3-inators.
#
...
for arg in "$@"; do
    case "$arg" in
        -h|--help)
            cat <<EOF
Build python3-inators as a Debian .deb via dpkg-buildpackage.

Usage: build.sh [-h|--help]

Environment overrides (rarely needed):
  INATORS_VERSION  upstream version (default: 2.1.1)
  INATORS_SHA256   sha256 of the upstream sdist (must match VERSION)

Output: \$REPO_ROOT/dist/python3-inators_<DEB_VERSION>_all.deb
EOF
            exit 0
            ;;
    esac
...
UPSTREAM_VERSION="${INATORS_VERSION:-2.1.1}"
SHA256="${INATORS_SHA256:-0a4a524e3fc5d976679401c4442cfd1a9673a5e5c248098ae346d3fd7659b456}"
UPSTREAM_URL="https://files.pythonhosted.org/packages/3c/23/2d738a9c18e5edfb08dc8cb830d8282549d16e27ed7db77d9df3e27640dc/inators-${UPSTREAM_VERSION}.tar.gz"

cd "$(dirname "$0")"
PKG_DIR="$(pwd)"
NAME="python3-inators"
REPO_ROOT="$(cd ../.. && pwd)"
...
ORIG_TARBALL="$WORKDIR/inators_${UPSTREAM_VERSION}.orig.tar.gz"
...
mv "$WORKDIR/inators-${UPSTREAM_VERSION}" "$SRC_DIR"
