...
### Housekeeping
- [ ] **2 packages ship no source package: `mesen2`, `tilemap-studio`** — their `build.sh` source pass `WARN`ed ("source package build failed") on the v1.5.35 publish, so they're in `dists/resolute/main/source/Sources.gz` as binary-only (51/53 sources present; binaries fine). `emit_source_package` (`scripts/lib-source-build.sh`) suppresses the `dpkg-source -b` stderr — reproduce each build.sh's source pass with stderr visible to get the real error (likely: `mesen2`'s appended-`.NET`-bundle ELF / large blob confuses `dpkg-source`; `tilemap-studio` may modify the tree pre-build). Fix so all packages publish sources. Low priority — graceful degradation worked as designed.
...
- [ ] **Get `apt.foundrylinux.org` tracked by Repology (as a producer)** — we fit the BackBox add-on-derivative pattern (an Ubuntu derivative tracked at `minpackages: 10`), so size is not a blocker. Order: ~~(1) publish source packages → `main/source/Sources.gz`~~ + ~~(2) `Origin`/`Label` = `Foundry Linux`~~ — **both ✅ live as of v1.5.35 (2026-06-21)**; (3) `scripts/generate-repology-ruleset.sh` emitting the `repology-rules` YAML from our `X-Repology-Project` fields (`setname`/`ignore`); (4) register on the [Debian Derivatives Census](https://wiki.debian.org/Derivatives/Census); (5) PR repo def → `repology-updater` + ruleset → `repology-rules`. (1)–(3) improve the repo regardless. See [investigation](docs/investigations/2026-06-21-repology-tracking-foundry-apt.md).
