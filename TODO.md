...
- [ ] **Add JRE 17 to foundry-core, JRE 21 to foundry-atelier** — Java runtimes are a baseline expectation for game-dev (LibGDX, Gradle). JRE 17 → `foundry-core` (all tiers + devbox); JRE 21 → `foundry-atelier` (intentional dep, not transient via ghidra). Adds ~90 MiB on ISO (anvil ~3.93 GiB). See [plan](docs/plans/2026-06-05-add-jre-17-to-foundry-core-jre-21-to-foundry-ateli.md).
...
- [ ] **⚠ Pre-release: remove live-session debug SSH** — the live-ssh hook (`foundry-iso/config/hooks/1200-live-ssh.hook.chroot`) bakes `root:foundry` + sshd into the **live ISO** (squashfs) for debugging. It does NOT reach installed systems (Calamares manages root normally; the `setRootPassword: false` that leaked it to installs was reverted in calamares-settings 1.0.30 — that was a backdoor and must never recur). Still a known root password on the bootable live image — **gate/remove before any public release.**
- [ ] **[verify] ISO local-debs publish-aware staging + always-local installer** — `build-iso.sh` now stages only local debs newer than the published apt repo (kills the Discover same-version "Refresh" noise) and always bundles the purged-on-install installer config (`calamares-settings-foundry-linux`) regardless of the published version. Implemented + committed; run step 4 (post-allowlist install: no same-version refresh on foundry pkgs, installer matches the local build) on the next install. See [plan](docs/plans/2026-06-10-iso-local-debs-publish-aware-staging.md).
...
- [ ] **Foundry-owned desktop theming (panel favorites, consolidate into foundry-kde-theme)** — move the panel favorites (remove Kate, add Blender) and the rest of the desktop default out of Kubuntu's `org.kubuntu.desktop.defaultPanel` template into our package. Approach not yet chosen (Foundry LAF vs complete appletsrc vs panel template); needs the real current `launchers=` / active LAF read first. See [plan](docs/plans/2026-06-10-foundry-kde-desktop-theming.md).
