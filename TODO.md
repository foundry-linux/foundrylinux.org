...
- [wip T3] **Finish the per-package-publish production proof, then land the prepared `apt.worldfoundry.org` port** — Phase 1 code is synced and the cold bootstrap is running, but the workflow token gets `AccessDenied` for `foundry-apt-dist`; authorize that bucket, prove hydrate + targeted publish + round-trip, then land the locally tested Phase 2 changes and authorize `worldfoundry-apt-dist`. See the [plan status](docs/plans/2026-08-05-per-package-publish.md#implementation-and-production-status-updated-2026-08-06).
...
- [ ] **xemu — check the 8 targets the default build skips** (`xcx16` Commander X16, `xzxspect` ZX Spectrum, `xrc2014`, `xc900`, `xpc`, `xrecpm`, `xkeyconfig`, `xep128old`). Upstream ships 15 targets; `make ARCH=native` builds 7. If the rest build cleanly they are more emulators at no packaging cost — the `.desktop` generation already picks up anything added to BINARIES. See [plan](docs/plans/2026-08-05-xemu-rename-and-upstream-pr.md).
