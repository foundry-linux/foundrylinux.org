...
- [verify T2] **Guarantee a full KDE config stack** — build-time assertion in `foundry-iso/scripts/build-iso.sh` (after `lb chroot`) checks the KDE config QML modules (`org.kde.kcmutils`, `org.kde.kquickcontrols`, `QtQuick/Dialogs`) are present in the chroot, so a future strip-list edit can't silently break plasmoid config dialogs. Guard comments added to hook 0020 + `strip.list.chroot.purge`; omit/ship rationale in [docs/investigations/2026-05-30-kde-app-kit.md](docs/investigations/2026-05-30-kde-app-kit.md). **Step 1 PASS on ISO 0.9.133 (2026-09-19: the assertion fired and passed in-build). Remaining: step 2 (negative test — purge kquickcontrols, rebuild, expect failure; ~90 min build), step 3 (atelier build), step 4 (boot smoke — blocked on the headless smoke item below).** See [plan](docs/plans/2026-05-30-full-kde-experience.md). _(Surfaced while testing the claude-usage KDE plasmoid live; that empty-config-dialog bug was a claude-usage bug, not a Foundry gap.)_
- [T2] **Headless variant of the ISO boot smoke test** — `foundry-iso/test/boot-smoke.sh` (and `task iso-qemu`) launch `qemu-system-x86_64 -display gtk,gl=on`, an interactive window, so no agent or CI job can run the KDE plan's step 4 (boot + live-ssh `qmllint` probe) without popping a GUI on the maintainer's desktop. Add a `--headless` mode (`-display none`, serial console, ssh-forward on a local port, bounded wait) that `task iso-smoke` uses by default when `DISPLAY`/`WAYLAND_DISPLAY` is unset or `HEADLESS=1`; keep the GTK path for humans. Unblocks step 4 above.

- [T4] **Kiosk mode (gamescope + wf-launcher)** — **BLOCKED on `wf-launcher`, which does not exist; re-confirmed 2026‑09‑03** (no source tree under `~`, no repo in `foundry-linux` or `wbniv`, no package in either apt repo, no renamed successor — `foundry-welcome` is an info panel, not a launcher). A locked-down session that boots straight into a game launcher; deferred from the [phase-3 ISO plan](docs/plans/2026-05-22-phase-3-foundry-iso.md). **Do not design around the absence** — the [kiosk-mode stub](docs/plans/2026-09-03-kiosk-mode.md) holds the re-runnable evidence check, the contract `wf-launcher` must meet to be a valid gamescope-session target, and the unblock gate; start there. Future phase, not blocking v1.
...
- [T1] **File Debian ITP for `xemu`** — GPL-2.0-or-later, [github.com/lgblgblgb/xemu](https://github.com/lgblgblgb/xemu). Check [wnpp](https://bugs.debian.org/cgi-bin/pkgreport.cgi?pkg=wnpp) for an existing RFP/ITP first (none found as of 2026-08-05). Neither `xemu` nor `xemu-xbox` is taken in Debian; we propose `xemu` for LGB's suite (its own .deb has claimed that name since 2016) and `xemu-xbox` for the unrelated original-Xbox emulator. Draft prepared at [docs/itp-drafts/xemu.md](docs/itp-drafts/xemu.md); send upstream a heads-up first, then only the BTS filing remains. See [rename plan](docs/plans/2026-08-05-xemu-rename-and-upstream-pr.md).
- [T3] **Audit the 30 vendored upstreams for their own Debian packaging** — every package predates the `/package` skill's new Step 2.5, so none was checked. Build `foundry-apt/scripts/audit-upstream-packaging.sh` (reuse each `build.sh`'s pinned fetch), run it, and record the result — positive or negative — in each ITP draft. Caught late on `x-emulators`, where upstream's own `.deb` script named the package `xemu` after we had already shipped `x-emulators`. See [plan](docs/plans/2026-08-05-audit-upstream-packaging.md).
- [T4] **xemu `xcx16` — finish the Commander X16 emulator** — it builds, but upstream calls it “very rudimentary” and recommends the official emulator. Implement the missing sound, SD-card, raster IRQ/border/video behavior, keyboard quality, and sane speed control; do not package it until upstream removes the usability warning.
- [T4] **xemu `xpc` — finish the experimental PC/Fake86 integration** — it builds, but its README says “Unfinished” and “Do not use this (yet)”. Reconcile it with upstream's Fake86 fork, complete BIOS/device behavior, and add boot-image tests before exposing it to users.
- [T4] **xemu `xzxspect` — make the ZX Spectrum target usable** — it builds, but upstream explicitly says it is “simply unusable”. Correct display and timing, implement program loading and sound, and validate 48K ROM execution before considering it for the package.
- [T4] **xemu `xep128old` — restore and reassess the legacy Enterprise target** — verify it in an environment with `libreadline-dev`, repair any remaining build failures, and compare it with the packaged rewritten `xep128`. Do not ship both unless the old early-alpha target retains distinct, tested functionality.
- [T3] **xemu `xkeyconfig` — restore the missing font link** — provide `font_16x16` through the current shared font mechanism and restore a clean link. Treat it as a keyboard-layout utility, not an eighth emulator; decide separately whether it belongs in the binary package.
- [T3] **xemu `xrc2014` — port to the current keyboard API** — replace the removed `CONFIG_KBD_ALSO_*_SDL_CALLBACK` interface and restore a clean native build; add a minimal RC2014 boot/console smoke test.
- [T3] **xemu `xrecpm` — port obsolete input and font integration** — migrate off the removed keyboard callback API and stop including `vgafonts.c` directly; request the font through `emutools.c`, restore the build, and smoke-test its CP/M console.
- [T3] **xemu `xc900` — repair the incomplete Z8000 implementation** — resolve the missing `z8010_init`/`z8010_reset` declarations or implementations and the missing `F_OVERFLOWSUB_BY16` flag calculation, then establish a basic Commodore 900 boot test.
...
### foundry-atelier ISO build ([plan](docs/plans/2026-05-22-phase-3-foundry-iso.md))

- **Build `foundry-atelier-1.0-amd64.iso`** — `task iso-build EDITION=atelier`; verify ghidra + full atelier package set present; sign + upload to R2. Target ~10 GB. Parked 2026-09-03, no rush — revisit whenever priorities point back at it.

### Migrate ISO hosting to Internet Archive ([investigation](docs/investigations/2026-05-22-iso-hosting.md))

- **[at v1.0.0]** — ~~provision IA S3 keys~~ ✅ done 2026-06-14 (`scripts/bootstrap-ia.sh` → `IA_S3_ACCESS_KEY`/`IA_S3_SECRET_KEY` in foundry-linux-secrets R2 + GH secrets). Remaining: add `scripts/upload-iso-ia.sh` (rclone → `s3.us.archive.org`), add Cloudflare Worker redirecting `iso.foundrylinux.org/*` → archive.org download URLs, update `publish.yml`. R2 stays for apt repos. **Parked 2026-09-03: hold until the distro is renamed** (the "Foundry Linux" name is being dropped — "Linux" is a trademark of the Linux Mark Institute — so `iso.foundrylinux.org`, the R2 bucket paths, and any archive.org identifiers this work would create should be cut over to the new name first rather than built under the old one and migrated twice).

## Done

- ✅ 2026-09-19 — [add-uv-to-base] uv 0.12.17 vendored + in foundry-core; live on apt v1.5.49, wald3n v0.0.430, devbox v0.0.7, ISO 0.9.133. All 10 verify steps PASS. See [plan](docs/plans/2026-09-19-add-uv-to-base.md).
...
- [x] 2026-09-03 — [per-package-publish-phase-2] Released `apt.worldfoundry.org` port: pushed `apt-v0.1.38`, live index went 14→15 packages, completeness gate passed (`15 local .debs, 14 live packages`), dist mirror seeded at R2 `.dist-cache/`. Run [33806221321](https://github.com/wbniv/worldfoundry.org/actions/runs/33806221321). See [plan](docs/plans/2026-08-05-per-package-publish.md).
- [x] 2026-08-30 — [package-losslesscut] `losslesscut 3.69.0-1foundry1` and `foundry-sprite 1.0.2` published in foundry-apt v1.5.47; live Ubuntu 26.04 install + GUI/sandbox/FFmpeg probes passed; wald3n inventory deployed and verified in v0.0.427; upstream notified in lossless-cut#3035 and the malformed FFmpeg RPATH reported in ffmpeg-builds#1. Debian RFP remains prepared but unfiled because email submission is prohibited. No ISO build was run. See [completion plan](docs/plans/2026-08-30-complete-losslesscut-publication.md).
- [x] 2026-08-05 — [per-package-publish phase 1 implementation] Synced R2 hydrate/persist wiring, multi-package targeted dispatches, and the fail-closed live-index completeness gate. Production proof remains open above because `foundry-apt-dist` currently returns `AccessDenied`; the R2 mirror is not yet operational. See [plan](docs/plans/2026-08-05-per-package-publish.md).
...
- 2026-06-14 — [package-bsnes-jg] packaged bsnes-jg 2.1.0 as a `.deb` (GPL-3.0, Jolly Good fork of bsnes v115; cycle-accurate SNES/SFC) — self-contained SDL frontend `/usr/bin/bsnes-jg` + game DB/desktop/icons, jgemu/jg headers make-installed at build time, deps resolved (SDL2/libsamplerate), `X-Repology-Project: bsnes-jg`; wired into `foundry-emulators-consoles` + `-heavy`, live on apt.foundrylinux.org. (Commit 06bf4aa; ITP still open below.)
...
<!-- triaged 2026-08-05: the audit-upstream-packaging deferrals remain in its plan; the CI guard is complete and the binary-name divergence was closed by decision on 2026-08-06. -->
