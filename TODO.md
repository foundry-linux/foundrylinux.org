...
- [ ] **drmon: land the cppdap/JsonCpp CMake fix upstream in `developer-resources-co/drdevtools`** — `devsys/tools/drmon/CMakeLists.txt` FetchContents cppdap without its JsonCpp submodule → `Json::Value` undefined at link. Fix = `set(CPPDAP_USE_EXTERNAL_JSONCPP_PACKAGE ON …)` + `find_package(jsoncpp REQUIRED)` + link `jsoncpp_lib` (needs `libjsoncpp-dev`; Ubuntu 26.04 ships 1.58.0a = the pinned cppdap tag). Diff prepared at `/tmp/drdevtools-pr` branch `fix/drmon-cppdap-jsoncpp-link`; we own the repo — **push pending Will's go-ahead** (auto-classifier blocked an unprompted push to main). See [plan](docs/plans/2026-06-12-package-pvsneslib.md#drmon).
- [ ] **drmon: drop the carry-forward sed once upstream is fixed** — re-pin `packages/drmon/build.sh` `DRDEVTOOLS_SHA`+`SHA256` to the fixed commit and delete the two jsoncpp `sed` patches (keep the `libs/` sed + `libjsoncpp-dev` build-dep); re-verify + re-publish. Optional further step: switch drmon to the system `libcppdap-dev` (drops FetchContent entirely).
