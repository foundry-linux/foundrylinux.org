...
- [T2] **wald3n `refresh-open-source-data.mjs` must read committed content, not the sibling working tree** — it scans `../foundrylinux.org/foundry-apt/packages` on disk, so any in-flight deletion there (seen 2026-09-19: the shared-electron removal) breaks `open-source:refresh` and the `package-publish:complete` guard. Read from `git archive HEAD` (or a `--ref` option) in `wald3n.com/scripts/`, and add the canonical-checkout regression check (reject a legacy path, compare git identity) that [pipeline-hardening](docs/plans/2026-08-06-package-publish-pipeline-hardening.md) step 4 calls for. Surfaced by [activate-wf-dispatch](docs/plans/2026-08-05-activate-wf-dispatch.md).
- [verify T2] **package-publish pipeline hardening** — verification run 2026-09-19 against the `uv` proof (`v1.5.49`): 7 PASS / 3 FAIL / 1 retired. The FAILs are steps 2–4, all missing tests (hydrate/persist fail-closed, marker lifecycle, wald3n canonical-checkout check) — tracked by the two items below; re-run steps 2–4 once they land, then promote. See [plan](docs/plans/2026-08-06-package-publish-pipeline-hardening.md).
