...
- [ ] **foundry-welcome** — QML/Kirigami welcome screen (3 pages: intro, tools, community); show-once sentinel; XDG autostart; masks plasma-welcome. Package in `foundry-apt/packages/foundry-welcome/`; drop .deb into `foundry-iso/config/includes.chroot/tmp/local-debs/` after build. See [plan](docs/plans/2026-05-24-foundry-welcome.md). **In progress (2026-05-25):** package structure + QML pages written; build + ISO integration pending.
