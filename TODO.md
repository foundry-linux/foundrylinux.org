...
- [ ] **Guarantee a full KDE config stack** — add a build-time assertion (in `build-iso.sh` after `lb chroot`) that the KDE config QML modules (`org.kde.kcmutils`, `org.kde.kquickcontrols`, `QtQuick/Dialogs`) are present in the chroot, so a future strip-list edit can't silently break plasmoid config dialogs. Audit (2026-05-30) found the current strip is conservative and these ARE present — this just locks it in. See [plan](docs/plans/2026-05-30-full-kde-experience.md). _(Surfaced while testing the claude-usage KDE plasmoid live; that empty-config-dialog bug was a claude-usage bug, not a Foundry gap.)_
