...
### Housekeeping
- [ ] **`new-web-apt-repo` skill: document source packages in the generated index** — the skill's `build-all.sh`/`publish-local.sh` templates now build + publish source packages, but its `gen/gen-index.py` index has no "Source packages" / `deb-src` block (foundry-apt's `generate-index.sh` grew one 2026-06-21 — [plan](docs/plans/2026-06-21-source-package-instructions.md)). Mirror it into the skill's `gen-index.py` so new repos document `apt-get source` too. (apt.worldfoundry.org's index stays binary-only until Phase 2 source-building is applied there.)
