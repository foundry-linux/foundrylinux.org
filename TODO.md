...
- [T1] **Add `ares`, `sameboy`, `mgba-qt` to `foundry-emulators-consoles`** — all three confirmed in Ubuntu 26.04 universe (`ares 147+dfsg-3`, `sameboy 1.0.2+ds-2`, `mgba-qt 0.10.5+dfsg-3build1`). `ares` closes **ColecoVision** (confirmed in its `--help` system list, alongside MSX/MSX2, SG-1000, Neo Geo Pocket, WonderSwan, Mega Drive/CD/32X); `sameboy` + `mgba-qt` close **Game Boy / GBC / GBA** as dedicated emulators (today GB is only incidental via `mednafen`/`mesen2`). One `debian/changelog` bump + `Depends:` + description edit, then `task build`. Small enough to ride the next release tag. See coverage table in [plan](docs/plans/2026-08-05-package-xemu.md).
- [T5] **Decide where `mame` belongs for console coverage** — MAME is the *only* packaged path to **PONG**, the **Magnavox/Philips Odyssey**, and the **Odyssey² / Videopac** (standalone `o2em` not in the archive), but it ships in `foundry-retro-tools`, not either emulator bucket. ~444 MB installed → plausibly `-heavy`, not the light bucket `foundry-core` pulls. **Unverified:** the `pong`/`odyssey`/`odyssey2`/`coleco`/`lynx`/`gameboy` driver names in Debian's `+dfsg1` MAME 0.285 build — a container `mame -listfull` probe returned zero lines; confirm before writing them into any package description.
