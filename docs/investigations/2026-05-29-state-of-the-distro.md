...
> | 6 | North‑star proposal stale | ✅ **Bannered (06‑04)** — moved to `docs/investigations/`, `status:` now reads SUPERSEDED, top banner enumerates divergences (foundry-dev→editions; AWS-SSM signing *replaced*; Steam/Sniper, per-game Distrobox, gamescope kiosk *deferred‑not‑dropped*; creative-distro scope expansion) |
...
6. ~~**🟡 The north‑star proposal is now a historical artifact.**~~ **✅ Reclassified + bannered (06‑04).** The proposal was moved `docs/plans/` → `docs/investigations/2026-05-16-foundry-linux-distro-proposal.md`, its `status:` set to SUPERSEDED, and a top banner now enumerates the divergences. **Important distinction (corrected 06‑04 — the original finding got this wrong):** of the proposal's unshipped items, only the **AWS‑SSM/OIDC/YubiKey two‑tier signing** was genuinely *replaced* (by single‑key GPG + R2 DR backup). **Steam/Sniper release containers, per‑game Distrobox, and the gamescope kiosk are still wanted — deferred, not dropped** (kiosk + Distrobox already in `TODO.md`; Steam/Sniper added to it 06‑04). `foundry-dev` — the centerpiece — no longer exists; it became `foundry-core` + `foundry-desktop`. ~20 art/audio/emulator/games metapackages appeared with no proposal basis (now documented as the creative‑distro scope expansion).
