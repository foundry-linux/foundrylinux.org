...
`foundry-setup/install.sh` is the Phase 0 setup script from the Foundry Linux distro proposal ([`docs/investigations/2026-05-16-foundry-distro-proposal.md`](../investigations/2026-05-16-foundry-distro-proposal.md)). It was supposed to install everything the Phase 1 APT metapackages will eventually install — including the retro-porting toolchain (MAME, `dasm`, `cc65` → ships `sim65` 6502 emulator + `da65` disassembler, `radare2`, etc., plus source-built Ghidra / f9dasm / vgmstream / libvgm / xa65). It currently doesn't: `install_apt_packages()` at [`install.sh:222-246`](../../foundry-setup/install.sh) only mirrors `Taskfile.yml:dev-setup` (engine build deps). That's the gap behind the original "which script installs the 6502 emulator(s)" question — there isn't one.
...
foundry-setup/
  ...
  install-foundry-engine-build-deps.sh    (NEW)
  install-foundry-blender.sh              (NEW)
  install-foundry-retro-tools.sh          (NEW — answers the 6502 question)
  install-foundry-android-dev.sh          (NEW)
  install-task.sh                              (NEW)
  install-foundry-dev.sh                  (NEW — orchestrator for the umbrella metapackage)
```

Each per-metapackage script is the Phase 0 expansion of one metapackage's `Depends:` and source-build-sidecar `Recommends:`. The orchestrator (`install-foundry-dev.sh`) calls the constituent scripts in dependency order, matching [`foundry-apt/packages/foundry-dev/DEBIAN/control`](../../foundry-apt/packages/foundry-dev/DEBIAN/control).
...
|---|---|---|
| `install-foundry-engine-build-deps.sh` | `build-essential` `cmake (>=3.22)` `libx11-dev` `libgl1-mesa-dev` `libglu1-mesa-dev` `gdb` `xxd` `python3` `pkg-config` `git` `curl` `wget` `unzip` | — |
| `install-foundry-blender.sh` | `blender (>=4.2)` `python3` `foundry-engine-build-deps` | invokes `wftools/wf_blender/install.sh` for the addon (Phase 1 `Recommends: foundry-blender-addon`) |
| `install-foundry-retro-tools.sh` | `mame` `mame-tools` `dasm` `cc65` `z80dasm` `z80asm` `radare2` `binwalk` `sox` `binutils-m68k-linux-gnu` | `ghidra` → `~/opt/ghidra-*/`; `f9dasm` → `~/opt/f9dasm/`; `vgmstream-cli` → `~/opt/vgmstream/`; `libvgm` → `~/opt/libvgm/`; `xa65` → `~/opt/xa65/` (per [arcade-tooling investigation](../investigations/2026-05-15-claude-arcade-tooling.md) §2 + §1 note line 46) |
| `install-foundry-android-dev.sh` | `openjdk-17-jdk` `adb` `google-android-ndk-r26c-installer` | — |
...
| `install-foundry-dev.sh` | composes the above in this order: engine-build-deps → task → blender → retro-tools → android-dev (optional) | calls each script as `bash install-<name>.sh "$@"` |
...
| Role | Calls |
|---|---|
| `play` | (none today — no `foundry-runtime` metapackage exists yet; warn and continue) |
| `game-dev` | `install-foundry-engine-build-deps.sh`, `install-task.sh`, `install-foundry-blender.sh`, `install-foundry-retro-tools.sh`, then clone `wf-games`, build wftools |
| `engine-dev` | `install-foundry-engine-build-deps.sh`, `install-task.sh`, `install-foundry-retro-tools.sh`, then clone `WorldFoundry.2026-new-level`, build wftools |
| `both` (default) | `install-foundry-dev.sh` (= engine-build-deps + blender + retro-tools + task), then clone both repos, build wftools |
| `maintainer` | `both` + `install-foundry-android-dev.sh` |
...
2. Handles `-h` / `--help` — prints "Installs the foundry-X metapackage's Phase 0 equivalent" + flag list + exits 0. Per `~/SRC/CLAUDE.md`: `-h` must short-circuit before any arg parsing or apt calls.
...
- `foundry-setup/lib.sh` — extracted helpers: color codes, `log_to_file`, `info`, `ok`, `warn`, `err`, `die`, `step`, `run`, `run_sudo`, `init_logging`. Source these from current `install.sh:50-93`.
- `foundry-setup/install-foundry-engine-build-deps.sh` — body extracted from `install.sh:222-246` (`install_apt_packages`).
- `foundry-setup/install-foundry-blender.sh` — `apt install blender python3` + invoke `wftools/wf_blender/install.sh` (current `install.sh:394-415` `install_blender_addon`).
- `foundry-setup/install-foundry-retro-tools.sh` — apt block from `docs/investigations/2026-05-15-claude-arcade-tooling.md:34-42` + `xa65` source-build fallback from line 46 + Ghidra/f9dasm/vgmstream/libvgm source-builds from §2 of the same doc. **This is the script that installs the 6502 emulator(s)** (`sim65` via `cc65`; full 6502 system emulation via MAME).
- `foundry-setup/install-foundry-android-dev.sh` — extracted from `Taskfile.yml:dev-setup` lines 249-251 (`openjdk-17-jdk adb google-android-ndk-r26c-installer`).
- `foundry-setup/install-task.sh` — body from `install.sh:281-295` (`install_task_runner`).
- `foundry-setup/install-foundry-dev.sh` — sequential calls to the constituent scripts.

**Modified:**

- `foundry-setup/install.sh` — replace the 14 step functions with: arg parsing, distro check (162-199), sudo check (201-217), `init_logging` (now in `lib.sh`), role-based dispatch to the per-metapackage scripts, then the non-metapackage Phase 0 steps that stay: `install_rust` (248-279), `clone_wf_repos` (297-365), `build_wftools` (367-392), `summary` (420-448). Net length: estimated 200-220 lines down from 475.

**Reused (no changes):**

- `wftools/wf_blender/install.sh` — invoked by `install-foundry-blender.sh`.
- `wftools/wf_blender/install_blender_mcp.sh` — optional, invoked by same.
- `Taskfile.yml:dev-setup` (236-251) — left as-is; it's a developer convenience that overlaps with `install-foundry-engine-build-deps.sh` + `install-foundry-android-dev.sh` but doesn't need to be a single canonical source anymore.
...
1. **6502 emulator install works in isolation.** On a fresh Ubuntu 26.04 VM:
    ...
    ```
    bash foundry-setup/install-foundry-retro-tools.sh
...
2. **Dry-run is clean.** `bash install-foundry-retro-tools.sh --dry-run` prints every `apt-get install` and every source-build `git clone` + `make` without executing. Same for each other per-meta script.

3. **Orchestrator wiring.** `bash install-foundry-dev.sh --dry-run` prints calls to each constituent script in the documented order.

4. **Role dispatch.** `bash install.sh --role engine-dev --dry-run` prints calls to engine-build-deps + task + retro-tools (skips blender, skips android-dev). `--role both --dry-run` calls `install-foundry-dev.sh`.
...
**Apt half: PASS (cross-plan evidence).** [`2026-05-18-foundry-apt-live-install-tests.md`](2026-05-18-foundry-apt-live-install-tests.md) (Status: Done, 5/5 pass) installs `foundry-retro-tools` from `apt.foundrylinux.org` on fresh Ubuntu 26.04, validating the `Depends:` (mame, mame-tools, dasm, cc65, z80dasm, z80asm, radare2, binwalk, sox, binutils-m68k-linux-gnu) and therefore `sim65`, `da65`, `mame`, `radare2`.
...
→ Open follow-up: package each of `xa65`, `f9dasm`, `libvgm`, `vgmstream`, `Ghidra` as real `.deb`s in `foundry-apt/packages/` (these would be the first non-metapackage `.deb`s in the repo). Once that lands, `foundry-retro-tools` Depends on them and Step 1 collapses to a pure live-install test.
...
```
$ bash foundry-setup/install-foundry-dev.sh --dry-run

━━━ foundry-dev: engine build deps ━━━
ℹ → install-foundry-engine-build-deps.sh --dry-run
━━━ foundry-dev: task runner ━━━
ℹ → install-task.sh --dry-run
━━━ foundry-dev: Blender ━━━
ℹ → install-foundry-blender.sh --dry-run
```
...
```
$ bash foundry-setup/install.sh --role engine-dev --force --dry-run
⚠ Ubuntu-family 25.10 — untested but --force is set
ℹ → install-foundry-engine-build-deps.sh --dry-run
ℹ → install-task.sh --dry-run
ℹ → install-foundry-retro-tools.sh --dry-run --force

$ bash foundry-setup/install.sh --role both --force --dry-run
ℹ → install-foundry-dev.sh --dry-run --force
```

**PASS** — `engine-dev` invokes engine-build-deps + task + retro-tools (skips blender, android-dev). `both` invokes the umbrella `install-foundry-dev.sh`. (Host is Ubuntu 25.10, hence `--force`; same dispatch on 24.04/26.04.)

### Step 5 — `-h` short-circuits

```
install.sh                                       rc=0  Foundry Linux setup script (Phase 0)
install-foundry-android-dev.sh             rc=0  Phase 0 installer for foundry-android-dev
install-foundry-blender.sh                 rc=0  Phase 0 installer for foundry-blender
install-foundry-dev.sh                     rc=0  Phase 0 installer for foundry-dev (umbrella metapackage)
install-foundry-engine-build-deps.sh       rc=0  Phase 0 installer for foundry-engine-build-deps
install-foundry-retro-tools.sh             rc=0  Phase 0 installer for foundry-retro-tools
...
Each `install-foundry-*.sh` body cleanly maps to `run_sudo apt-get install -y <metapackage>` once `apt.foundrylinux.org` is configured. Source-build sidecars (xa65, f9dasm, libvgm, vgmstream, Ghidra) are guarded by `[[ -d ~/opt/<tool> ]]` and will be replaced by `.deb`s once foundry-apt CI builds them.
