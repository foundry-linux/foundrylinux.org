...
### 1. New script: `foundry-setup/setup-foundry-apt-source.sh`
...
The local `foundry-retro-tools/debian/control` already has `libvgm` in `Depends:`.
`libvgm_*.deb` is built and in `dist/` but not yet published. The retro-tools collapse in step 3
will cause `apt install foundry-retro-tools` to fail unless libvgm is resolvable.

Action: bump `foundry-retro-tools` changelog (move `libvgm` into `Depends:` was already
...
| Script | Before | After |
|---|---|---|
| `install-foundry-engine-build-deps.sh` | `apt-get install -y build-essential cmake libx11-dev ...` (13 packages) | `apt-get install -y worldfoundry-engine-build-deps` |
| `install-foundry-blender.sh` | `apt-get install -y blender python3` | `apt-get install -y worldfoundry-blender` |
| `install-foundry-retro-tools.sh` | `apt-get install -y mame mame-tools ...xa65` (12 packages) | `apt-get install -y foundry-retro-tools` (resolves same 12 + f9dasm + libvgm transitively) |
| `install-foundry-android-dev.sh` | `apt-get install -y openjdk-17-jdk adb ...` | `apt-get install -y worldfoundry-android-dev` |
| `install-foundry-dev.sh` | orchestrator: calls the others | `apt-get install -y worldfoundry-dev` (single call gets everything) |

**`--apt-only` remains valid** after the collapse — it now suppresses the remaining source-build
sidecars in `install-foundry-retro-tools.sh`: ghidra and vgmstream (libvgm moves from
...
**Source-build sidecars stay** in `install-foundry-retro-tools.sh` for now — Ghidra and
...
**Also fix stale comment** on line 7 of `install-foundry-retro-tools.sh`:
`DEBIAN/control` → `debian/control`.

### 4. Drop the f9dasm source-build sidecar

`install-foundry-retro-tools.sh` lines ~135-140 (the `# --- f9dasm` block — clone + make) become dead code once Phase 0 apt-installs `foundry-retro-tools` (which `Depends:` on `f9dasm`). Delete it. Same pattern as the xa65 sidecar retirement in [`2026-05-18-retire-xa65.md`](2026-05-18-retire-xa65.md), just for f9dasm.

### 5. Update worldfoundry-dev metapackage `Depends:` (if needed)

`worldfoundry-dev`'s `debian/control` already `Depends:` on `worldfoundry-engine-build-deps`, `worldfoundry-blender`, `foundry-retro-tools`, etc. — so `apt install worldfoundry-dev` recursively pulls everything. No change needed; verifying in step 6.

### 6. `install-foundry-dev.sh` collapses to a single apt call
...
on line 14 which incorrectly says `foundry-dev` (the real package is `worldfoundry-dev`).
...
2. **First run configures the source.**
    ...
    bash foundry-setup/setup-foundry-apt-source.sh
...
4. **`apt-cache policy foundry-retro-tools` resolves from foundry-apt.**

    ```bash
    apt-cache policy foundry-retro-tools
...
5. **Collapsed retro-tools script installs the same package set as the old one.**
    ...
    Diff should be empty (modulo `foundry-retro-tools` itself appearing in the new run as an installed metapackage).

6. **Source-build sidecars for libvgm, vgmstream, Ghidra still execute.**

    ```bash
    bash foundry-setup/install-foundry-retro-tools.sh --dry-run | grep -E "git clone"
    # Expect: 3 git clone lines (libvgm, vgmstream, ghidra). NO f9dasm.
    ```

7. **`install-foundry-dev.sh --dry-run` shows the single `apt install foundry-dev` (instead of dispatching to constituents).**

8. **End-to-end live install in fresh ubuntu:26.04 container** — `bash install.sh --role engine-dev --force --apt-only` succeeds and `foundry-retro-tools` is installed via apt.
