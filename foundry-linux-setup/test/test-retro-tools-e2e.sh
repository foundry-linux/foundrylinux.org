#!/usr/bin/env bash
# End-to-end test: install foundry-linux-retro-tools in a fresh Ubuntu 26.04
# container and verify every tool from the metapackage Depends: actually runs.
#
# Usage: bash foundry-linux-setup/test/test-retro-tools-e2e.sh [--version 26.04]
#
...
exec $RUNTIME run --rm \
    -v "$(pwd):/work:ro" \
    "ubuntu:${VERSION}" \
    bash /work/foundry-linux-setup/test/test-retro-tools-e2e-inner.sh
